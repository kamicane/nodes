...
var iterators = {
  ...
  '~>': function(node, visitor) { // same as many children, but avoid block statements.
    return node.traverse(function(value, key, skip) {
      if (
        types.BlockStatement.test(value) ||
        types.ForInStatement.test(value) ||
        types.ForOfStatement.test(value) ||
        types.ForStatement.test(value)
      ) return skip;
      return visitor.call(node, value, key);
    }, true);
  },
...
var pseudos = {
  ...
  declaration: function(node, value) {
    ...
    if (types.Function.test(parentNode)) {
      ...
      if (parentNode.rest === node) return true;
      if (~parentNode.params.indexOf(node)) return true;
      if (~parentNode.defaults.indexOf(node)) return true;
