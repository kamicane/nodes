...
var iterators = {
  ...
  '~>': function(node, visitor) { // same as many children, but avoid block statements.
    return node.traverse(function(value, key, skip) {
      if (types.BlockStatement.test(value)) return skip;
      return visitor.call(node, value, key);
    }, true);
  },
  ...
  '=>': function(node, visitor) { // same as many children, but avoid functions.
    return node.traverse(function(value, key, skip) {
      if (types.Function.test(value)) return skip;
      return visitor.call(node, value, key);
    }, true);
  },

  '+>': function(node, visitor) { // same as many children, but avoid non-arrow functions.
    return node.traverse(function(value, key, skip) {
      if (types.FunctionExpression.test(value) || types.FunctionDeclaration.test(value)) return skip;
      return visitor.call(node, value, key);
    }, true);
  },
  ...
  '>>': function(node, visitor) { // same as many children, but avoid classes.
    return node.traverse(function(value, key, skip) {
      if (types.Class.test(value)) return skip;
      return visitor.call(node, value, key);
    }, true);
  },
