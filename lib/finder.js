...
function matchContextParts(cache, context, parts, index) {
  ...
  if (cacheContext[string]) {
    ...
    // console.log('using', context.constructor.name + '@' + uid, string);
  } else {
    ...
    // console.warn('setting', context.constructor.name + '@' + uid, string);
...
function match(node, part, key) {
  ...
  if (isPrimitive(node)) {
    ...
    if (part.tag !== '*' && (key === void 0 || key != part.tag)) return false;
    return true;
  }
  ...
  if (part.tag !== '*') {
    ...
    if (key === -1 || key === void 0 || key != part.tag) return false;
  }
