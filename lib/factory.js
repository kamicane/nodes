...
var createObjectAccessor = function(proto, key, accepts) {
  var validate = createNodeValidator([ accepts ]);
...
var defineList = function(type, key, accepts) {
...
function defineNode(type) {
    ...
    for (var key in keys) {
      ...
      switch (desc.kind) {
        ...
        case "object":
          ...
          if (value !== undefined) this[key] = new types[desc.accepts](value);
        break;
  ...
  for (var key in keys) {
    ...
    switch (desc.kind) {
      case "list":
        defineList(type, key, accepts);
      break;
      case "node":
        createNodeAccessor(proto, key, accepts);
      break;
      case "object":
        createObjectAccessor(proto, key, accepts);
      break;
      case "native":
